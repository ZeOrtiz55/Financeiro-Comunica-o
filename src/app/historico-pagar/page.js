...
// ÍCONES MODERNOS
import { Bell, Menu, ArrowLeft, FileText, CheckCircle, Download } from 'lucide-react'
...
export default function HistoricoPagar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLista(data || [])
      setTimeout(() => setLoading(false), 800) // Timeout leve para curtir a tela de loading
    }
    fetchData()
  }, [router])

  const gridLine = '1px solid #cbd5e1';
  ...
  return (
    <div style={{ 
        ...
        // NOVA IMAGEM: ESCRITÓRIO CORPORATIVO LOUNGE
      ...
      >
        <div style={{ opacity: isSidebarOpen ? 1 : 0, transition:'0.2s', whiteSpace:'nowrap' }}>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <button onClick={() => router.push('/historico-pagar')} style={{ background:'#0f172a', color:'#fff', border:'none', padding:'12px 15px', textAlign:'left', fontWeight:'900', cursor:'pointer', fontSize:'12px' }}>CONTAS PAGAR</button>
                <button onClick={() => router.push('/historico-receber')} style={{ background:'none', color:'#475569', border:'none', padding:'12px 15px', textAlign:'left', fontWeight:'700', cursor:'pointer', fontSize:'12px' }}>CONTAS RECEBER</button>
            </nav>
        </div>
        {!isSidebarOpen && (
      ...
      <main style={{ marginLeft: '85px', flex: 1, padding: '50px', zIndex: 1, position: 'relative' }}>
        <header style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'60px' }}>
            <div>
                <h1 style={{ fontWeight: '900', color: '#0f172a', margin: 0, fontSize:'32px', letterSpacing:'-1.5px' }}>HISTÓRICO DE PAGAMENTOS</h1>
                <div style={{ width: '60px', height: '4px', background: '#ef4444', marginTop: '12px' }}></div>
            </div>
        ...
        <div style={{ background: 'rgba(252, 252, 252, 0.9)', backdropFilter: 'blur(10px)', borderRadius: '15px', border: '1px solid #cbd5e1', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.05)' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', textAlign:'left' }}>
            <thead style={{ background: '#f8fafc', borderBottom: '1px solid #cbd5e1' }}>
              <tr>
                ...
                <th style={{ fontSize:'10px', fontWeight:'900', color:'#94a3b8', letterSpacing:'1px' }}>FORNECEDOR</th>
                <th style={{ fontSize:'10px', fontWeight:'900', color:'#94a3b8', letterSpacing:'1px' }}>VALOR PAGO</th>
            ...
            <tbody>
              {lista.map((item, index) => (
                <tr key={item.id} style={{ 
                  ...
                  <td style={{ fontSize:'14px', fontWeight:'700', color:'#0f172a' }}>{item.fornecedor.toUpperCase()}</td>
                  ...
                  <td style={{ textAlign:'center' }}>
                    <div style={{ display:'flex', gap:'8px', justifyContent:'center' }}>
                      {item.anexo_nf && (
                        <a href={item.anexo_nf} target="_blank" title="Ver Nota Fiscal" style={{ padding:'8px', borderRadius:'8px', background:'#f1f5f9', color:'#475569', border:'1px solid #cbd5e1' }}>
                            <FileText size={16} />
                        </a>
                      )}
                      {item.anexo_comprovante && (
                        <a href={item.anexo_comprovante} target="_blank" title="Ver Comprovante" style={{ padding:'8px', borderRadius:'8px', background:'#f0fdf4', color:'#166534', border:'1px solid #bcf0da' }}>
                            <CheckCircle size={16} />
                        </a>
                      )}
                      {item.anexo_boleto && (
                        <a href={item.anexo_boleto} target="_blank" title="Ver Boleto" style={{ padding:'8px', borderRadius:'8px', background:'#fff', color:'#0f172a', border:'1px solid #cbd5e1' }}>
                            <Download size={16} />
                        </a>
                      )}
                    </div>
          ...
          {lista.length === 0 && (
            <div style={{padding:'80px', textAlign:'center'}}>
                <p style={{ fontSize:'12px', fontWeight:'700', color:'#94a3b8', letterSpacing:'1px' }}>NENHUM REGISTRO ENCONTRADO NO HISTÓRICO.</p>
            </div>
          )}
        </div>
      </main>

      {/* ÍCONE DE NOTIFICAÇÃO FIXO E ELEGANTE (Padronizado com a Home) */}
      <div style={{ position: 'fixed', top: '50px', right: '50px', zIndex: 1200, display: 'flex', gap: '20px', alignItems: 'center' }}>
