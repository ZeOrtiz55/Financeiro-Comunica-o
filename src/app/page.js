...
function ChatChamado({ chamadoId, userProfile }) {
 ...
 useEffect(() => {
   ...
   const fetchMessages = async () => {
       const { data } = await supabase.from('mensagens_chat').select('*').eq('chamado_id', idReal).order('created_at', { ascending: true });
       setMensagens(data || []);
   }
   fetchMessages();

   const channel = supabase.channel(`chat_card_v2_${idReal}`).on('postgres_changes', { 
     event: 'INSERT', schema: 'public', table: 'mensagens_chat', filter: `chamado_id=eq.${idReal}` 
   }, payload => { 
       // Realtime cuida da atualização do estado para evitar duplicação
 ...
 const enviar = async (e) => {
   ...
   // Apenas insere, o Realtime cuida de exibir na tela
   ...
   if (error) alert("Erro ao salvar chat interno: " + error.message)
 }
...
function ChatFlutuante({ userProfile, isOpen, setIsOpen }) {
 ...
 const carregarMensagensGerais = async () => {
 ...
 useEffect(() => {
   ...
   carregarMensagensGerais();

   const channel = supabase.channel('realtime_global_chat_v8').on('postgres_changes', { 
     event: 'INSERT', schema: 'public', table: 'mensagens_chat' 
   }, (payload) => { 
       if (!payload.new.chamado_id && !payload.new.pagar_id && !payload.new.receber_id && !payload.new.rh_id) {
           setMensagens(prev => [...prev, payload.new]);
       }
   }).subscribe();
 ...
 const enviar = async (e) => {
   ...
   const payload = { 
     texto: t, usuario_nome: userProfile.nome, usuario_id: userProfile.id, 
     chamado_id: null, pagar_id: null, receber_id: null, rh_id: null,
     data_hora: new Date().toISOString(), visualizado_por: [{ id: userProfile.id, nome: userProfile.nome }] 
   };

   // Apenas insere, evita duplicidade no estado local
   const { error } = await supabase.from('mensagens_chat').insert([payload]);
   if (error) {
       ...
       <div style={{ position: 'absolute', bottom: '95px', right: 0, width: '500px', height: '750px', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', borderRadius: '35px', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 70px rgba(0,0,0,0.3)', border:'1px solid #e2e8f0', overflow:'hidden' }}>
          ...
          <div ref={scrollRef} style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '15px' }}>
             {mensagens.map((m, idx) => {
               ...
               return (
                 <div key={m.id || idx} style={{ alignSelf: souEu ? 'flex-end' : 'flex-start', maxWidth:'80%' }}>
...
export default function Home() {
 ...
 const addToast = (notif) => {
   ...
   try {
     const somPref = userProfile?.som_notificacao || 'notificacao.mp3';
 ...
 const abrirCardViaNotificacao = async (chamadoId) => {
   if (!chamadoId) {
       setIsGlobalChatOpen(true);
       return;
   }
   const { data } = await supabase.from('Chamado_NF').select('*').eq('id', chamadoId).single();
   if (data) setTarefaSelecionada({ ...data, gTipo: 'boleto' });
 };
 ...
 useEffect(() => {
   if (userProfile) {
     carregarDados();
     const channelChat = supabase.channel('notif_hub_global_final').on('postgres_changes', { 
       event: 'INSERT', schema: 'public', table: 'mensagens_chat' 
     }, async payload => {
         ...
         if (payload.new.chamado_id) {
            ...
            idCardLink = payload.new.chamado_id;
            titulo = "MENSAGEM NO CARD";
            mensagem = `PROCESSO: #${payload.new.chamado_id} | CLIENTE: ${card?.nom_cliente?.toUpperCase()}`;
            detalhes = `Texto: "${payload.new.texto}"`;
         }

         setNotificacoes(prev => [{ titulo, mensagem, detalhes, idCardLink, data: new Date().toISOString() }, ...prev]);
         addToast({ tipo: 'chat', titulo, mensagem, detalhes, idCardLink });
     }).subscribe();

     const channelMove = supabase.channel('move_hub_global_final').on('postgres_changes', { 
       event: 'UPDATE', schema: 'public', table: 'Chamado_NF' 
         ...
         if (payload.old.status !== payload.new.status) {
           ...
           setNotificacoes(prev => [{ titulo, mensagem, detalhes, idCardLink: info.id, data: new Date().toISOString() }, ...prev]);
           addToast({ tipo: 'movimento', titulo, mensagem, detalhes, idCardLink: info.id });
         }
         carregarDados();
     }).subscribe();
 ...
 const handleUpdateFile = async (id, table, field, file) => {
   ...
   await supabase.from(table).update({ [field]: data.publicUrl }).eq(idReal);
 ...
 const handleGerarBoletoFinanceiro = async (id) => {
   ...
   try {
     ...
     if (typeof id === 'string' && id.includes('_p')) {
         ...
         await supabase.from('Chamado_NF').update({ status: 'enviar_cliente', tarefa: 'Enviar Boleto para o Cliente', setor: 'Pós-Vendas', anexo_boleto: data.publicUrl }).eq(idReal);
     }
   ...
   <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Montserrat, sans-serif', background: 'transparent' }}>
     ...
     <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 10000, display: 'flex', flexDirection: 'column', gap: '15px' }}>
       {toasts.map(t => (
         <NotificationToast 
           ...
           onClick={() => abrirCardViaNotificacao(t.idCardLink)}
         />
       ))}
     </div>
     ...
     <main style={{ marginLeft: isSidebarOpen ? '320px' : '85px', flex: 1, padding: '50px', zIndex: 1, position: 'relative', background: 'transparent', transition: '0.4s ease' }}>
           ...
           <div style={{ display:'flex', gap:'35px', alignItems:'center', position:'relative' }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: '20px', background: 'rgba(255,255,255,0.6)', padding: '12px 25px', borderRadius: '25px', border: '1px solid #cbd5e1', boxShadow: '0 8px 20px rgba(0,0,0,0.05)' }}>
                 <div style={{ width: '65px', height: '65px', borderRadius: '20px', overflow: 'hidden', background: '#000', border: '2px solid #fff', boxShadow: '0 5px 15px rgba(0,0,0,0.1)' }}>
                   {userProfile?.avatar_url ? <img src={userProfile.avatar_url} style={{ width:'100%', height:'100%', objectFit:'cover'}} alt="Avatar" /> : <User color="#fff" style={{padding:'12px'}} size={40}/>}
                 </div>
                 ...
                 {showNotifMenu && (
                   <div onMouseLeave={() => setShowNotifMenu(false)} style={{ position: 'absolute', top: '55px', right: 0, background: 'rgba(255,255,255,0.98)', backdropFilter: 'blur(10px)', padding: '25px', borderRadius: '20px', boxShadow: '0 20px 50px rgba(0,0,0,0.15)', border: '1px solid #cbd5e1', zIndex: 2000, width: '400px', maxHeight: '500px', overflowY: 'auto' }}>
                     ...
                     {notificacoes.length > 0 ? notificacoes.map((n, i) => (
                       <div 
                         key={i} 
                         onClick={() => abrirCardViaNotificacao(n.idCardLink)}
                         style={{ padding: '15px 0', borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}
                       >
               ...
               {/* BOTÃO NOVO CHAMADO CORRIGIDO: NOME BRANCO, MENU COM FONTE PRETA */}
               <div style={{ position: 'relative' }}>
                 <button onClick={() => setShowNovoMenu(!showNovoMenu)} style={{ background:'#0f172a', color:'#fff', border:'none', padding:'20px 40px', borderRadius:'15px', fontWeight:'900', cursor:'pointer', fontSize:'16px', display:'flex', alignItems:'center', gap:'12px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)' }}>
                   <PlusCircle size={26} strokeWidth={2.5} color="#fff" /> <span style={{ color: '#fff' }}>NOVO CHAMADO</span>
                 </button>
                 {showNovoMenu && (
                   <div onMouseLeave={() => setShowNovoMenu(false)} style={{ position:'absolute', top:'85px', right: 0, background:'#fff', borderRadius:'20px', boxShadow: '0 30px 60px rgba(0,0,0,0.2)', zIndex:2000, width:'320px', border:'1px solid #e2e8f0', overflow:'hidden' }}>
                     <div onClick={() => router.push('/novo-chamado-nf')} style={{ padding:'25px', cursor:'pointer', fontSize:'16px', fontWeight:'500', color: '#000', borderBottom:'1px solid #f1f5f9' }} className="hover-item">CHAMADO DE BOLETO</div>
                     <div onClick={() => router.push('/novo-pagar-receber')} style={{ padding:'25px', cursor:'pointer', fontSize:'16px', fontWeight:'500', color: '#000', borderBottom:'1px solid #f1f5f9' }} className="hover-item">CONTAS PAGAR / RECEBER</div>
       ...
       <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
           ...
           <div style={{flex: 1, minWidth: '380px', display: 'flex', flexDirection: 'column', gap: '30px'}}><div style={{padding: '15px', textAlign: 'center', fontWeight: '400', fontSize: '24px', color: '#0f172a'}}>CONTAS PAGAR / RECEBER</div>
               {(listaPagar || []).map(t => (
                 <div key={t.id} onClick={() => setTarefaSelecionada({ ...t, gTipo: 'pagar' })} className="task-card">
                   <div style={{ background: '#ef4444', padding: '25px', color: '#fff' }}><h4 style={{ margin: 0, fontSize: '28px', fontWeight: '500' }}>{t.fornecedor?.toUpperCase()}</h4></div>
                   <div style={{ padding: '25px' }}>
                     <span className="payment-badge" style={{background:'#000', color:'#fff'}}>{t.forma_pagamento?.toUpperCase() || 'PAGAMENTO'}</span>
                     <div style={{marginTop:'15px', fontSize:'22px', fontWeight:'500', color: '#0f172a'}}>R$ {t.valor}</div>
                   </div>
                 </div>
               ))}
               {(listaReceber || []).map(t => (
                 <div key={t.id} onClick={() => setTarefaSelecionada({ ...t, gTipo: 'receber' })} className="task-card">
                   <div style={{ background: '#3b82f6', padding: '25px', color: '#fff' }}><h4 style={{ margin: 0, fontSize: '28px', fontWeight: '500' }}>{t.cliente?.toUpperCase()}</h4></div>
                   <div style={{ padding: '25px' }}>
                     <span className="payment-badge" style={{background:'#000', color:'#fff'}}>{t.forma_pagamento?.toUpperCase() || 'RECEBIMENTO'}</span>
                     <div style={{marginTop:'15px', fontSize:'22px', fontWeight:'500', color: '#0f172a'}}>R$ {t.valor}</div>
                   </div>
                 </div>
               ))}
     ...
     <style jsx global>{`
       ...
       * { font-weight: 400 !important; }
       ...
       .hover-item:hover { background: #f8fafc; color: #0f172a; }
     `}</style>
   </div>
 )
}