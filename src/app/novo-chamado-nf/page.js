...
export default function NovoChamadoNF() {
  ...
  useEffect(() => { setUsuariosFiltrados(todosUsuarios.filter(u => u.funcao === setor)) }, [setor, todosUsuarios])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tarefaI = isPix ? 'Validar Recebimento Pix' : (formData.tarefa || 'Gerar Boleto')

      const { error } = await supabase.from('Chamado_NF').insert([{
        ...formData,
        setor, status: statusI, tarefa: tarefaI,
        anexo_nf_servico: urlS, anexo_nf_peca: urlP, comprovante_pagamento: urlPix,
        vencimento_boleto: formData.data_primeira_parcela
      }])

      if (!error) { alert("Chamado gerado!"); router.push('/') }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', background:'#f8fafc' }}>
      <div style={{ background: '#fff', padding: '40px', borderRadius: '45px', width: '100%', maxWidth: '550px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {(tipoNF === 'servico' || tipoNF === 'ambas') && (
                <div style={{ background:'#f8fafc', padding:'15px', borderRadius:'20px', border:'1px solid #eee' }}>
                  <label style={{fontSize:'10px', fontWeight:'bold'}}>NOTA DE SERVIÇO</label>
                  <input type="text" placeholder="Nº Nota" required style={glassInput} onChange={(e)=>setFormData({...formData, num_nf_servico: e.target.value})} />
                  <input type="file" required style={{marginTop:'10px'}} onChange={(e)=>setFileServico(e.target.files[0])} />
                </div>
              )}
              {(tipoNF === 'pecas' || tipoNF === 'ambas') && (
                <div style={{ background:'#f8fafc', padding:'15px', borderRadius:'20px', border:'1px solid #eee' }}>
                  <label style={{fontSize:'10px', fontWeight:'bold'}}>NOTA DE PEÇAS</label>
                  <input type="text" placeholder="Nº Nota" required style={glassInput} onChange={(e)=>setFormData({...formData, num_nf_peca: e.target.value})} />
                  <input type="file" required style={{marginTop:'10px'}} onChange={(e)=>setFilePeca(e.target.files[0])} />
                </div>
              )}
            </div>
          )}
          ...
          <input type="number" placeholder="Valor R$" required style={glassInput} onChange={(e)=>setFormData({...formData, valor_servico: e.target.value})} />

          <select required style={glassInput} onChange={(e)=>setFormData({...formData, forma_pagamento: e.target.value})}>
            <option value="">FORMA DE PAGAMENTO</option>
            <option value="Pix">À vista Pix (Comprovante obrigatório)</option>
          ...
          <button disabled={loading} style={{ background: '#22c55e', color: '#fff', padding: '18px', borderRadius: '20px', border: 'none', fontWeight: '900', cursor: 'pointer' }}>
